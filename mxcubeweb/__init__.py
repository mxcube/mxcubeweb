...
def build_server_and_config(test=False, argv=None):
    ...
    try:
        # MXCUBEApplication.init_logging() is what attaches the "HWR" logger's
        # handlers, but it runs later, once the server object it depends on
        # exists, which also depends on HWR is initialised. Give "HWR" a
        # plain console handler before everything is initilized so that
        # messages apear in the console,
