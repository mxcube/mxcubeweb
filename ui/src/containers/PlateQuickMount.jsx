...
const validate = (values,props)=>{
  ...
  if(!values.pin_num){
    ...
    const pinNumRegex = /^\d{1,2}$/;    // must be like 1 or 11
    
    if(!pinNumRegex.test(values.pin_num)){
      errors.pin_num = 'Must be like 1 or 01'
    }
...
const PlateQuickMount =(props) =>{

    const dispatch = useDispatch();
    const formSelector = formValueSelector('plateQuickMount')
    ...
    const mount = () => {
      handleSubmit((values)=>{
        ...
        // 
        const pinNumRegex_if_XX = /^\d{2}$/;       // mxst like 01 or 11


        if (!pinNumRegex_if_XX.test(pin_num_input)){
          // console.log('change pin_num_input like: 1 -> 01')
          // console.log(pin_num_input)
