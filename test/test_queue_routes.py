...
def test_add_and_get_task(client, add_sample, add_task):
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and len(
        json.loads(rv.data).get('1:05')['tasks']) == 1)
...
def test_add_and_edit_task(client, add_sample, add_task):
    ...
    rv = client.post(('/mxcube/api/v0.1/queue/{}/{}').format(queue_id, task_queue_id),
                     data=json.dumps(task_to_update),
                     content_type='application/json'
                     )
    assert (rv.status_code == 200 and json.loads(rv.data).get(
        'parameters')[parameter_to_update] == parameter_update_value)
...
def test_queue_start(client, add_sample, add_task):
    ...
    rv = client.put('/mxcube/api/v0.1/queue/start',
    ...
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('queueStatus') == 'QueueRunning')
...
def test_queue_stop(client, add_sample, add_task):
    ...
    rv = client.get('/mxcube/api/v0.1/queue_state')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('queueStatus') == 'QueueStopped')
...
def test_queue_delete_item(client, add_sample, add_task):
    ...
    rv = client.post('/mxcube/api/v0.1/queue/delete',
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and len(
        json.loads(rv.data).get('1:05')['tasks']) == 0)
...
def test_queue_enable_item(client, add_sample):
    ...
    test_disable = {'qidList': [queue_id],
                    'enabled': False
                    }

    rv = client.post('/mxcube/api/v0.1/queue/set_enabled',
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('1:05')['checked'] == False)
...
def test_queue_swap_task_item(client, add_sample, add_task):
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and len(
        json.loads(rv.data).get('1:05')['tasks']) == 2)

    rv = client.post(('/mxcube/api/v0.1/queue/{}/{}/{}/swap').format('1:05', 0, 1),
                     content_type='application/json'
                     )
    assert rv.status_code == 200
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(rv.data).get(
        '1:05')['tasks'][0]['parameters']['kappa'] == 90)
...
def test_queue_move_task_item(client, add_sample, add_task):
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and len(
        json.loads(rv.data).get('1:05')['tasks']) == 3)

    rv = client.post(('/mxcube/api/v0.1/queue/{}/{}/{}/move').format('1:05', 0, 2),
                     content_type='application/json'
                     )
    assert rv.status_code == 200
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(rv.data).get(
        '1:05')['tasks'][2]['parameters']['kappa'] == 0)
...
def test_queue_move_task_item_fail(client, add_sample, add_task):
    ...
    rv = client.post(('/mxcube/api/v0.1/queue/{}/{}/{}/move').format('1:05', 2, 2),
                     content_type='application/json'
                     )
    rv = client.get('/mxcube/api/v0.1/queue')

    assert (rv.status_code == 200 and json.loads(rv.data).get(
        '1:05')['tasks'][2]['parameters']['kappa'] == 180)
...
def test_queue_set_sample_order(client, add_sample):
    ...
    rv = client.post('/mxcube/api/v0.1/queue/sample-order',
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('sample_order')[1] == '1:06')
...
def test_get_default_char_acq_params(client):
    ...
    assert (rv.status_code == 200 and json.loads(
        rv.data) == default_char_acq_params)
...
def test_get_default_xrf_parameters(client):
    ...
    assert (rv.status_code == 200 and json.loads(
        rv.data) == default_xrf_parameters)


def test_set_automount(client):
    """Test if we can set automount for samples."""
    rv = client.post('/mxcube/api/v0.1/queue/automount',
                     data=json.dumps(True),
                     content_type='application/json')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('automount') == True)


def test_set_num_snapshots(client):
    """Test if we can set num of snapshots for acq."""
    rv = client.put('/mxcube/api/v0.1/queue/num_snapshots',
                    data=json.dumps({'numSnapshots': 2}),
                    content_type='application/json')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('numSnapshots') == 2)


def test_set_group_folder(client):
    """Test if we can set group folder."""
    rv = client.post('/mxcube/api/v0.1/queue/group_folder',
                     data=json.dumps({'path': 'tmp/'}),
                     content_type='application/json')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('path') == 'tmp/')


def test_set_autoadd(client):

    rv = client.post('/mxcube/api/v0.1/queue/auto_add_diffplan',
                     data=json.dumps(True),
                     content_type='application/json')
    assert (rv.status_code == 200 and json.loads(
        rv.data).get('auto_add_diffplan') == True)
