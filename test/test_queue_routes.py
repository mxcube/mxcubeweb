...
def test_add_and_edit_task(client, add_sample, add_task):
    ...
    assert (rv.status_code == 200 and json.loads(rv.data).get('parameters')[parameter_to_update] == parameter_update_value)

def test_queue_start(client, add_sample, add_task):
    """Test if we can start the queue. The queue requires a sample and a task to start which are added by fixtures."""
...
def test_queue_swap_task_item(client, add_sample, add_task):
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(rv.data).get('1:05')['tasks'][0]['parameters']['kappa'] == 90)
...
def test_queue_move_task_item(client, add_sample, add_task):
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(rv.data).get('1:05')['tasks'][2]['parameters']['kappa'] == 0)
...
def test_queue_move_task_item_fail(client, add_sample, add_task):
    ...
    assert (rv.status_code == 200 and json.loads(rv.data).get('1:05')['tasks'][2]['parameters']['kappa'] == 180)
...
def test_queue_set_sample_order(client, add_sample):
    ...
    rv = client.get('/mxcube/api/v0.1/queue')
    assert (rv.status_code == 200 and json.loads(rv.data).get('sample_order')[1] == '1:06')
...
def test_set_autoadd(client):
    ...
    rv = client.post('/mxcube/api/v0.1/queue/auto_add_diffplan',
                     data=json.dumps(True),
                     content_type='application/json')
    assert (rv.status_code == 200 and json.loads(rv.data).get('auto_add_diffplan') == True)
